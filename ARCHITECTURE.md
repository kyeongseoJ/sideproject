...
- Mission Phase 3는 `/api/missions/settings`, `/api/missions/today`, `/api/missions/today/recommendations`를 통해 설정과 오늘 후보를 제공하며, 같은 날짜의 후보는 PostgreSQL `USER_MISSION`에서 복원한다.
- Mission Phase 4는 `/api/user-missions/{userMissionId}/select|cancel|replace|complete`로 사용자별 상태를 변경한다. Service가 사용자·설정·대상 미션을 잠그고 PostgreSQL 활성 슬롯과 상태 로그를 함께 갱신한다.
