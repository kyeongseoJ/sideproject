...
- 기본 미션 Catalog는 M001~M200의 200개이며 8개 Category에 각각 24~26개를 배분한다. 교체 전 행은 과거 이력 참조를 위해 삭제하지 않고 `enabled=N`으로 보존한다.
...
- Mission Catalog는 기존 필드를 재사용하고 `actionType`, `creativityLevel`, `unpredictability`, `comfortZoneDistance`, `costLevel`, `tags`를 추가한다. `durationLevel`은 `estimatedMinutes`에서 파생한다. BASE Seed 태그는 M001~M200 식별자와 한글 태그를 허용하고 LLM 태그는 대문자 영문 규칙을 유지한다.
